...
local pop_settings = sbar.add("item", "wifi.settings", {
  ...
  click_script = "open 'x-apple.systempreferences:com.apple.Network-Settings.extension'; "
    .. bin.sketchybar .. " --set wifi popup.drawing=off",
})

local function update()
  sbar.exec(bin.ifconfig .. " en0 2>/dev/null | awk '/status:/{print $2}'", function(status)
  ...
  sbar.exec(bin.ipconfig .. " getifaddr en0 2>/dev/null", function(ip)
    ip = (ip or ""):gsub("%s", "")
